...
using Penrose.Extensions;
using Penrose.Extensions.Extensions;
...
namespace Penrose.Application.Strategies.Chats
{
  public class ChatDataStrategy : DataStrategy<Chat>, IChatDataStrategy
  {
    ...
    private readonly ILogger<ChatDataStrategy> _logger;

    public ChatDataStrategy(
        IMapper mapper,
        IPenroseDbContext dbContext,
        ILogger<ChatDataStrategy> logger) : base(dbContext)
    {
    ...
    public async Task<Chat> InsertOneAsync(Chat chat, CancellationToken cancellationToken)
    {
      try
      {
        await _chatDb.AddAsync(chat, cancellationToken);
        await PenroseDbContext.SaveChangesAsync(cancellationToken);

        return (Chat) chat.Clone();
      }
      catch (Exception ex)
      {
        _logger.LogCritical(ex, "Failed to Insert one user.");
    ...
    public async Task<IEnumerable<ChatDto>> FindUserChatsAsync(Guid userId, CancellationToken cancellationToken)
    {
      return await _chatDb
          ...
          {
            Id = chat.Id,
            Properties = _mapper.Map<ChatPropertiesDto>(chat.Properties),
            UnreadMessages = chat.Messages.Count(x => !x.IsSeen),
            CreatedAt = chat.CreatedAt,
            UpdatedAt = chat.UpdatedAt,
            Participants = chat.Participants.Select(chatParticipant => _mapper.Map<UserDto>(chatParticipant.User)),
